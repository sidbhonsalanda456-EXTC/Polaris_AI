...
def train_demand_model(df, features):
    X = df[features].values
    y = df["demand_actual_kw"].values
...
def predict_demand(model, features_array):
    predictions = model.predict(features_array)
    return np.clip(predictions, 20, 200)
...
if __name__ == "__main__":
    ...
    df = load_data()
    df = preprocess(df)
    _, _, demand_f = get_feature_sets()
    model, metrics, X_test, y_test, y_pred = train_demand_model(df, demand_f)
