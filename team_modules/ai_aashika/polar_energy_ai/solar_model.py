...
def train_solar_model(df, features):
    X = df[features].values
    y = df["solar_actual_kw"].values
...
def predict_solar(model, features_array):
    predictions = model.predict(features_array)
    return np.clip(predictions, 0, 300)


if __name__ == "__main__":
    from preprocess import load_data, preprocess, get_feature_sets, split_data
    df = load_data()
    df = preprocess(df)
    solar_f, _, _ = get_feature_sets()
    model, metrics, X_test, y_test, y_pred = train_solar_model(df, solar_f)
    print("Solar model trained!")
